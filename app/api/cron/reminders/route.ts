...
const BATCH_SIZE = 50;
const CONCURRENCY = 10; // Reminders are cheaper than summaries (no AI call)
  ...
  while (hasMore) {
    ...
    for (let i = 0; i < proUsers.length; i += CONCURRENCY) {
      ...
      const results = await Promise.allSettled(
        chunk.map(user => processReminder(user, supabase, resend, now, thresholdStr, cooldownDate))
      );
...
async function processReminder(
  user: { id: string; first_name: string | null; last_reminder_sent_at: string | null; email_preferences: unknown },
  supabase: any,
  resend: Resend,
  now: Date,
  thresholdStr: string,
  ...
  if (prefs && prefs.reminders === false) return 'skipped';

  // Skip if reminder sent recently
  if (user.last_reminder_sent_at && new Date(user.last_reminder_sent_at) > cooldownDate) return 'skipped';
  ...
  if (totalRecent > 0) return 'skipped';

  // Find days since last log (check all 3 tables)
  const [{ data: lastSupplement }, { data: lastSymptom }, { data: lastHabit }] = await Promise.all([
    supabase.from('supplement_logs').select('date').eq('user_id', user.id).order('date', { ascending: false }).limit(1).single(),
    supabase.from('symptom_logs').select('date').eq('user_id', user.id).order('date', { ascending: false }).limit(1).single(),
    supabase.from('habit_logs').select('date').eq('user_id', user.id).order('date', { ascending: false }).limit(1).single(),
  ]);

  const dates = [lastSupplement?.date, lastSymptom?.date, lastHabit?.date].filter(Boolean) as string[];
  const mostRecentDate = dates.length > 0 ? dates.sort().reverse()[0] : null;

  const daysSinceLastLog = mostRecentDate
    ? Math.floor((now.getTime() - new Date(mostRecentDate + 'T00:00:00').getTime()) / (1000 * 60 * 60 * 24))
  ...
  await supabase.from('profiles').update({ last_reminder_sent_at: now.toISOString() }).eq('id', user.id);
